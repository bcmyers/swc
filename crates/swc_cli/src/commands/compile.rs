...
fn resolve_output_file_path(
    ...
    let base = RelativePath::new(base.as_str());
...
impl CompileOptions {
        ...
        match (collect_stdin_input(), &self.files[..]) {
            ...
            (Some(stdin_input), _) => {
                ...
                let source_file = {
                    let filename = if options.filename.is_empty() {
                        ...
                        FileName::Real(PathBuf::from(options.filename.clone()))
                    };
                    COMPILER.cm.new_source_file(filename, stdin_input)
                };
                Ok(vec![InputContext {
                    options,
                    source_file,
            ...
            (None, _) => self
                ...
                .map(|file_path| {
                    ...
                    let source_file =
                        COMPILER.cm.load_file(file_path.as_ref()).with_context(|| {
    ...
    fn execute_inner(&self) -> anyhow::Result<()> {
        let execute = |compiler: Arc<Compiler>, source_file: Arc<SourceFile>, options: Options| {
            try_with_handler(
                compiler.cm.clone(),
                HandlerOpts {
                ...
                |handler| compiler.process_js_file(source_file, handler, &options),
            )
        };
            ...
            let outputs = inputs
                ...
                .map(
                    |InputContext {
                         ...
                         options,
                         ..
                     }| execute(compiler, source_file, options),
                )
                .collect::<Result<Vec<_>, _>>()?;

            fs::create_dir_all(path.parent().expect("Parent should be available"))?;
        ...
        inputs.into_par_iter().try_for_each(
            |InputContext {
                ...
                execute(compiler, source_file, options).and_then(|output| {
    ...
    fn get_files_list<'a>(&'a self, _include_dotfiles: bool) -> anyhow::Result<Vec<Cow<'a, Path>>> {
        ...
        let mut output: Vec<Cow<'_, Path>> = Vec::new();
